    ...
    public function generateFinancialReportPDF($startDate, $endDate, $reportType = 'summary') {
        try {
            ...
            if (file_exists($logoPath)) {
                // preserve current page pointer
                $pw = $this->pdf->getPageWidth();
                $ph = $this->pdf->getPageHeight();
                $w = $pw * 0.6; // 60% of page width
                $h = 0; // auto
                $x = ($pw - $w) / 2;
                $y = ($ph - ($w * 0.4)) / 2; // approximate center (logo aspect assumed)
                $this->pdf->SetAlpha(0.08);
                $this->pdf->Image($logoPath, $x, $y, $w, $h, '', '', '', false, 300, '', false, false, 0);
                $this->pdf->SetAlpha(1);
            }
            ...
            // Timestamp footer (explicit)
            $this->pdf->Ln(6);
            $this->pdf->SetFont('helvetica', 'I', 9);
            $this->pdf->Cell(0, 6, 'Generated by ATIERA Financial Management System on ' . date('M j, Y H:i:s'), 0, 1, 'C');
