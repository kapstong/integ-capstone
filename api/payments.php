...
function postPaymentReceivedJournalEntry($db, $paymentId, $amount) {
    ...
    $cashAccountRow = $db->select("SELECT id FROM chart_of_accounts WHERE account_code = '1001' LIMIT 1");
    $cashAccountId = $cashAccountRow[0]['id'] ?? null;
    $entryNumber = generateJournalEntryNumber($db, $cashAccountId, date('Y-m-d'));
